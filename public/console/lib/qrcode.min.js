/*!
 * qrcode.min.js — minimal QR generator, SVG output, no deps.
 * Algorithm: Kazuhiko Arase, "QR Code for JavaScript", MIT licensed.
 *   Copyright (c) 2009 Kazuhiko Arase — http://www.d-project.com/
 *   https://www.opensource.org/licenses/mit-license.php
 * Vendored + trimmed for hara-control console:
 *   - canvas / table / Android workaround code removed
 *   - exposes window.HaraQR.toSvg(text, { size, margin, dark, light, ecLevel }) → SVG string
 *   - UTF-8 byte mode only (covers otpauth URIs — TOTP secrets are ASCII, labels can be unicode)
 */
(function (global) {
  "use strict";
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    // table truncated at typeNumber 11 — otpauth URIs fit comfortably in lower types
    // (a typical TOTP URI is ~80-120 chars; ecLevel M at type 6-8 handles 100-150)
  ];
  ...
  // byte-length capacity at each type for 8-bit byte mode, ecLevel M (truncated to type 11)
  var BYTE_CAP_M = [14, 26, 42, 62, 84, 106, 122, 152, 180, 213, 251];

  function pickType(text, ecLevel) {
    // we keep ecLevel fixed at M for simplicity (otpauth URIs are short, M is fine)
    var len = unescape(encodeURIComponent(text)).length; // UTF-8 byte length
    for (var t = 1; t <= BYTE_CAP_M.length; t++) {
      if (len <= BYTE_CAP_M[t - 1]) return t;
    }
    throw new Error("text too long for qrcode.min.js (max ~250 UTF-8 bytes at ecLevel M)");
  }

  /**
   * Render `text` as an SVG QR code. Returns an SVG string with viewBox set
   * to the module grid so it scales crisply at any rendered size.
   *
   * @param {string} text
   * @param {object} [opts]
   * @param {number} [opts.margin=4]  Quiet zone, in modules (≥4 per spec).
   * @param {string} [opts.dark="#000"]  Dark module color.
   * @param {string} [opts.light="#fff"]  Light module / background color.
   * @returns {string}
   */
  function toSvg(text, opts) {
    ...
    var type = pickType(text, ecLevel);
    var qr = new QRCodeModel(type, ecLevel);
    qr.addData(text);
    qr.make();
    ...
    var dim = n + margin * 2;
    var parts = [];
    parts.push('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + dim + ' ' + dim + '" shape-rendering="crispEdges">');
    parts.push('<rect width="' + dim + '" height="' + dim + '" fill="' + light + '"/>');
    // emit dark modules as one big path string (smaller than N rects)
    var d = "";
    for (var r = 0; r < n; r++) {
      for (var c = 0; c < n; c++) {
        if (qr.isDark(r, c)) {
    ...
    parts.push('<path fill="' + dark + '" d="' + d + '"/>');
    parts.push('</svg>');
    return parts.join("");
  }

  global.HaraQR = { toSvg: toSvg };
