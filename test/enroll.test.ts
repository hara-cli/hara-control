...
type Tok = { id: string; deviceId: string; tokenHash: string; gatewayKeyId: string; model: string; revokedAt: Date | null };
...
function fakePrisma() {
  ...
  const prisma = {
    ...
    enrollCode: {
      ...
      update: async ({ where: { id }, data }: { where: { id: string }; data: Partial<Code> }) => {
